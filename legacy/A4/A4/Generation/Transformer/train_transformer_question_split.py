...
from A4.Generation.Transformer.TransformerSeq2Seq import TransformerSeq2Seq
...
def generate(model, src, sos_id, eos_id, max_len=50):
    model.eval()
    with torch.no_grad():
        src = src.unsqueeze(0)  # [1, src_len]
        generated = [sos_id]
        for _ in range(max_len):
            tgt = torch.LongTensor(generated).unsqueeze(0).to(src.device)
            output = model(src, tgt)
            next_token = output[0, -1].argmax().item()
            generated.append(next_token)
            if next_token == eos_id:
                break
        return generated[1:]  # 去掉<sos>


def evaluate_generation(model, dataset, device, max_len=50):
    correct = 0
    total = 0
    for src, tgt in dataset:
        src = src.to(device)
        tgt = tgt.tolist()
        pred = generate(model, src, dataset.sos_id, dataset.eos_id, max_len=max_len)

        if dataset.pad_id in tgt:
            tgt = tgt[:tgt.index(dataset.pad_id)]
        if dataset.eos_id in tgt:
            tgt = tgt[:tgt.index(dataset.eos_id)]

        if pred == tgt[1:]:  # 跳过<sos>
            correct += 1
        total += 1
    return correct / total
...
def main():
    data_path = '../../geography/question_split'
    batch_size = 32
    embed_dim = 256
    lr = 1e-4
    epochs = 30
    ...
    model = TransformerSeq2Seq(
        src_vocab_size=len(train_set.word2id),
        tgt_vocab_size=len(train_set.target2id),
        embed_dim=embed_dim
    ).to(device)
    ...
    best_dev_acc = 0
    model_path = './Transformer_best_question_model.pt'
    print("\n\U0001f9e0 正在训练 Transformer 模型（question split）")

    for epoch in range(1, epochs + 1):
        train_acc = train(model, train_loader, optimizer, criterion, device)
        dev_acc = evaluate(model, dev_loader, criterion, device)
        print(f"Epoch {epoch}: \U0001f3cb️ Train Acc = {train_acc:.4f} | \U0001f3af Dev Acc = {dev_acc:.4f}")
    ...
    print("\n 正在进行句子级别生成评估...")
    sentence_acc = evaluate_generation(model, dev_set, device)
    print(f"✅ 句子级别准确率（完全匹配）: {sentence_acc:.4f}")
